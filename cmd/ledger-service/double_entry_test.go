...
func TestAuditVerificationChainIntegrity(t *testing.T) {
	...
	mt.Run("VerifyValidAuditChain", func(mt *mtest.T) {
		...
		doc1 := bson.D{
			{Key: "_id", Value: id1},
			{Key: "sequence_number", Value: int64(1)},
			{Key: "idempotency_key", Value: "k-1"},
			{Key: "source_wallet_id", Value: "alice"},
			{Key: "destination_wallet_id", Value: "bob"},
			{Key: "amount", Value: int64(100)},
			{Key: "currency", Value: "USD"},
			{Key: "timestamp", Value: ts1},
			{Key: "postings", Value: bson.A{
				bson.D{
					{Key: "account_id", Value: "alice"},
					{Key: "account_type", Value: "CUSTOMER_LIABILITY"},
					{Key: "direction", Value: "DEBIT"},
					{Key: "amount", Value: int64(100)},
					{Key: "currency", Value: "USD"},
				},
				bson.D{
					{Key: "account_id", Value: "bob"},
					{Key: "account_type", Value: "CUSTOMER_LIABILITY"},
			...
			{Key: "previous_hash", Value: GenesisHash},
			{Key: "entry_hash", Value: hash1},
		}

		doc2 := bson.D{
			{Key: "_id", Value: id2},
			{Key: "sequence_number", Value: int64(2)},
			{Key: "idempotency_key", Value: "k-2"},
			{Key: "source_wallet_id", Value: "bob"},
			{Key: "destination_wallet_id", Value: "charlie"},
			{Key: "amount", Value: int64(50)},
			{Key: "currency", Value: "USD"},
			{Key: "timestamp", Value: ts2},
			{Key: "postings", Value: bson.A{
				bson.D{
					{Key: "account_id", Value: "bob"},
					{Key: "account_type", Value: "CUSTOMER_LIABILITY"},
			...
			{Key: "previous_hash", Value: hash1},
			{Key: "entry_hash", Value: hash2},
		}
	...
	mt.Run("DetectTamperedEntryInChain", func(mt *mtest.T) {
		...
		docTampered := bson.D{
			...
			{Key: "previous_hash", Value: GenesisHash},
			{Key: "entry_hash", Value: "corrupted_hash_value_12345"},
		}
